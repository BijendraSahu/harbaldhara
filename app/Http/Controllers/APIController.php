...
//
//        }
        $points = DB::select("select sum(points) as Points,`user_id`, created_time, DATE_FORMAT(created_time,'%Y-%m-%d') as payment_date from `gain_points` where `user_id` = $user_id group by payment_date");
        if (count($points) > 0) {
